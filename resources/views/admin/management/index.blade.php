                                ...
                                <tr class="odd:bg-white even:bg-gray-100">
                                    ...
                                    <td class="whitespace-nowrap text-gray-600 border border-gray-300 px-4 py-2">
                                        <div class="flex justify-center items-center gap-2">
                                            <a href="{{ route('admin.mgmt.show', $user->id) }}" class="text-[#4A95FD] hover:bg-[#4A95FD] hover:text-white rounded-md px-2 py-1">
                                                <i class="fa-solid fa-pen-to-square"></i>
                                            </a>
                                            <span class="text-gray-300">|</span>
                                            <a href="{{ route('admin.mgmt.form_password', $user->id) }}" class="text-[#FFB300] hover:bg-[#FFB300] hover:text-white rounded-md px-2 py-1">
                                                <i class="fa-solid fa-key"></i>
                                            </a>
                                            <span class="text-gray-300">|</span>
                                            <form action="{{ route('admin.mgmt.delete', $user->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <button class="text-[#E24A36] hover:bg-[#E24A36] hover:text-white rounded-md px-2 py-1">
        ...
        <script>
            $(document).ready(function() {
                $('#adminTable').DataTable({
                    ...
                    "columnDefs": [
                    { "orderable": false, "targets": -1 } // -1 berarti kolom terakhir (Aksi)
                    ],
                });
            });
        </script>
