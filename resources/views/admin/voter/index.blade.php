            ...
            <div class="flex flex-col bg-white/30 backdrop-blur-lg rounded-xl shadow-lg p-6">
                <div class="bg-[#926AE1]/20 rounded-lg shadow-lg p-6">
                    ...
                    @if (Auth::user()->role == 'admin')
                        <div
                            class="mt-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-3 md:gap-4 ">
                                ...
                                <div class="flex flex-col sm:flex-row gap-3 md:gap-4">
                                    <div class="relative">
                                        <form action="{{ route('voter') }}" method="GET">
                                            <i
                                                class="fa fa-vote-yea absolute left-4 top-1/2 -translate-y-1/2 text-gray-400"></i>
                                            <select placeholder="Validasi" type="select" name="validation" id="validation"
                                                ...
                                                required autocomplete="off" onchange="this.form.submit()">
                                                <option value="" disabled selected>Status Vote</option>
                                                <option value="" {{ request('validation') == '' ? 'selected' : '' }}>
                                                    Semua</option>
                                                <option value="belum"
                                                    {{ request('validation') == 'belum' ? 'selected' : '' }}>Belum Memilih
                                                </option>
                                                <option value="sudah"
                                                    {{ request('validation') == 'sudah' ? 'selected' : '' }}>Sudah Memilih
                                                </option>
                                            </select>
                                        </form>
                                    </div>
                                        ...
                                        <div x-data="{ open: false }" class="relative">
                                            ...
                                            <div x-show="open"
                                                ...
                                                <div
                                                    ...
                                                    <p class="mb-6 font-medium text-gray-600 text-center text-sm">Yakin
                                                        untuk
                                                        menghapus semua data pemilih? <br> Saat ini memilih data dari:<br>
                                                        <span
                                                    ...
                                                    <form action="{{ route('voter.truncate') }}" method="POST">
                                                        ...
                                                        <div class="flex flex-row gap-2 justify-between">
                                                            <button type="button" @click="open = false"
                                                                ...
                                                                <span
                                                                    class="text-white font-semibold text-sm">Kembali</span>
                                                            </button>
                                                            <button type="submit"
                                                                class="px-5 py-2.5 bg-[#E24A36] rounded-lg">
                                                                <span
                                                                    class="text-[white] font-semibold text-sm">Hapus</span>
                                                            </button>
                                                        </div>
                                                    </form>
                                                </div>
                ...
                <div class="overflow-x-auto text-sm">
                    <div class="flex flex-col lg:flex-row items-center gap-4 mt-0.5 mb-8 text-gray-500">
                        <i class="fa-solid fa-filter fa-sm lg:fa-md"></i>
                        <p>Filter Print Berdasarkan: </p>

                        <div class="">
                            <form action="{{ route('voter.export.pdf') }}" method="GET"
                                class="flex flex-col lg:flex-row gap-4">
                                <!-- Filter Kelas -->
                                <div class="relative">
                                    <i
                                        class="fa fa-chalkboard-user absolute left-4 top-1/2 -translate-y-1/2 text-gray-400"></i>
                                    <select name="class" id="class"
                                        class="text-sm w-full h-14 pl-12 pr-12 text-gray-500 placeholder:text-gray-300 border border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm"
                                        autocomplete="off">
                                        <option value="" {{ request('class') == '' ? 'selected' : '' }}>Semua Kelas
                                        </option>
                                        @foreach ($classes as $class)
                                            <option value="{{ $class }}"
                                                {{ request('class') == $class ? 'selected' : '' }}>{{ $class }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <!-- Filter Jurusan -->
                                <div class="relative">
                                    <i class="fa fa-school absolute left-4 top-1/2 -translate-y-1/2 text-gray-400"></i>
                                    <select name="vocation" id="vocation"
                                        class="text-sm w-full h-14 pl-12 pr-12 text-gray-500 placeholder:text-gray-300 border border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm"
                                        autocomplete="off">
                                        <option value="" {{ request('vocation') == '' ? 'selected' : '' }}>Semua
                                            Jurusan</option>
                                        @foreach ($vocations as $vocation)
                                            <option value="{{ $vocation }}"
                                                {{ request('vocation') == $vocation ? 'selected' : '' }}>
                                                {{ $vocation }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <button type="submit"
                                    class="w-max-1xl h-full font-semibold flex items-center gap-3 px-4 py-3 lg:px-6 lg:py-4 bg-[#1D7AFC] text-white rounded-md hover:bg-[#1766D4]">
                                    <span class="text-sm lg:text-base">Print PDF</span>
                                                    ...
                                                    <div x-show="open"
                                                        ...
                                                        <div
                                                            ...
                                                            <p class="mb-6 font-medium text-gray-600 text-center">Yakin
                                                                ...
                                                                dari:<br><span
                                                                    class="font-semibold text-[#4F22AA]">({{ $voter->name }},
                                                                    {{ $voter->class }})</span>
                                                            </p>
